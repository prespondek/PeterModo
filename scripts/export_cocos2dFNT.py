...
#Title:         Export to FNT format
#Descrition:    Export Mesh Sheet to .fnt Format
...
if not lx.eval("query scriptsysservice userValue.isdefined ? fntfilepath"):
    lx.eval("user.defNew name:fntfilepath type:string life:temporary")
    lx.eval("user.value fntfilepath \"C:"")
lx.out(lx.eval("user.value vmffilepath ?"))

lx.eval("dialog.setup fileSave")
lx.eval("dialog.title \"FNT Save Location...\"")
lx.eval("dialog.fileTypeCustom vmf \"Cocos2d Font Format\" \"*.fnt\" fnt")
lx.eval("dialog.result \"" + lx.eval("user.value fntfilepath ?") + "\"")
try:
    lx.eval("dialog.open")
except:
    sys.exit("LXe_ABORT")
    
fnt_file = lx.eval("dialog.result ?")
lx.eval("user.defDelete name:fntfilepath")
lx.eval("user.defNew name:fntfilepath type:string life:temporary")
lx.eval("user.value fntfilepath \"" + map_file + "\"")
